// libkotfmod_CoreChannelGroup.cpp: [Core API] ChannelControl class
...
dllx double lkf_coreChannelGroup_exists(double cchg) {
	return (
		cchg >= 0.0
	&&	std::floor(cchg) == cchg
	&&	Global::CoreChannelGroups.find(static_cast<std::size_t>(cchg)) != Global::CoreChannelGroups.end()
	) ? 1.0 : 0.0;
}

dllx double lkf_coreChannelGroup_stop(double cc) {
	if (lkf_coreChannelGroup_exists(cc)) {
		auto& ccref{ Global::CoreChannelGroups[static_cast<std::size_t>(cc)] };
...
dllx double lkf_coreChannelGroup_set3DConeOrientation(double cc, FMOD_VECTOR* vec) {
	if (lkf_coreChannelGroup_exists(cc)) {
		...
		if (vec != nullptr) stack_vec = *vec;
		auto result{ ccref->set3DConeOrientation(&stack_vec) };
...
dllx double lkf_coreChannelGroup_get3DConeOrientation(double cc, FMOD_VECTOR* retmem) {
	if (lkf_coreChannelGroup_exists(cc)) {
		auto& ccref{ Global::CoreChannelGroups[static_cast<std::size_t>(cc)] };
		FMOD_VECTOR stack_vec{ 0.f, 0.f, 0.f };
...
dllx double lkf_coreChannelGroup_release(double cc) {
	if (lkf_coreChannelGroup_exists(cc)) {
		auto& ccref{ Global::CoreChannelGroups[static_cast<std::size_t>(cc)] };
		auto result{ ccref->release() };

		if (LKF_OK(result)) {
			lkf_T_pop_back_smart(Global::CoreChannelGroups, static_cast<std::size_t>(cc), Global::CoreChannelGroupLast);
			return 1.0;
		}
...
char temp_getName[2048]{ '\0' };

dllx const char* lkf_coreChannelGroup_getName(double cc) {
	if (lkf_coreChannelGroup_exists(cc)) {
		auto& ccref{ Global::CoreChannelGroups[static_cast<std::size_t>(cc)] };
		memset(temp_getName, '\0', sizeof(temp_getName));
		auto result{ ccref->getName(temp_getName, sizeof(temp_getName) - 1) };
...
dllx double lkf_coreChannelGroup_getNumChannels(double cc) {
	if (lkf_coreChannelGroup_exists(cc)) {
		auto& ccref{ Global::CoreChannelGroups[static_cast<std::size_t>(cc)] };
		int num{ 0 };
		auto result{ ccref->getNumChannels(&num) };
...
dllx double lkf_coreChannelGroup_getChannel(double cc, double index) {
	if (lkf_coreChannelGroup_exists(cc)) {
		...
		auto result{ ccref->getChannel(static_cast<int>(index), &channel) };
...
dllx double lkf_coreChannelGroup_getNumGroups(double cc) {
	if (lkf_coreChannelGroup_exists(cc)) {
		auto& ccref{ Global::CoreChannelGroups[static_cast<std::size_t>(cc)] };
		int num{ 0 };
		auto result{ ccref->getNumGroups(&num) };
...
dllx double lkf_coreChannelGroup_getGroup(double cc, double index) {
	if (lkf_coreChannelGroup_exists(cc)) {
		auto& ccref{ Global::CoreChannelGroups[static_cast<std::size_t>(cc)] };
		FMOD::ChannelGroup* cgr{ nullptr };
		auto result{ ccref->getGroup(static_cast<int>(index), &cgr) };

		if (LKF_OK(result)) {
			std::size_t i{ lkf_T_push_back_smart(Global::CoreChannelGroups, cgr, Global::CoreChannelGroupLast) };
			return static_cast<double>(i);
		}
...
dllx double lkf_coreChannelGroup_getParentGroup(double cc) {
	if (lkf_coreChannelGroup_exists(cc)) {
		auto& ccref{ Global::CoreChannelGroups[static_cast<std::size_t>(cc)] };
		FMOD::ChannelGroup* cgr{ nullptr };
		auto result{ ccref->getParentGroup(&cgr) };
...
dllx double lkf_coreChannelGroup_addDSP(double cc, double index, double cdsp) {
	if (lkf_coreChannelGroup_exists(cc) && lkf_coreDSP_exists(cdsp)) {
		auto& ccref{ Global::CoreChannelGroups[static_cast<std::size_t>(cc)] };
